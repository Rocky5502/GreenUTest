...
class ULTAdapter(TaskAdapter):
    ...
    """
    def __init__(self, path, benchmark_name: str = "ult"):
        self.path = Path(path)
        self.benchmark_name = benchmark_name
        self._reference_tests = {}
    def _rows(self):
    ...
    def tasks(self):
        ...
        for i,row in enumerate(self._rows()):
            ...
            yield Task(tid,self.benchmark_name,str(row.get("prompt") or "Generate unit tests for the supplied function."),code,row.get("repo") or row.get("repository"),float(cx) if cx is not None else None,{"upstream":safe_meta})
...
from .models import (
    FakeModel, ModelBackend, TransformersLocalModel, build_local_model_from_config,
    build_model_backend_from_config, OpenAIResponsesModel, AnthropicMessagesModel, GoogleGenAIModel,
)
